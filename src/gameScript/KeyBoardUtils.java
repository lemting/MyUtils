	...
	public static void keyCombination(String comb, int key) {
		...
		if(comb != null && comb.length() > 0) {
			for(char c: comb.toCharArray()) {
				switch (c) {
					case 'w': if(!keys.contains(KeyEvent.VK_WINDOWS)) keys.add(KeyEvent.VK_WINDOWS); break;
					case 'c': if(!keys.contains(KeyEvent.VK_CONTROL)) keys.add(KeyEvent.VK_CONTROL); break;
					case 'a': if(!keys.contains(KeyEvent.VK_ALT)) keys.add(KeyEvent.VK_ALT); break;
					case 's': if(!keys.contains(KeyEvent.VK_SHIFT)) keys.add(KeyEvent.VK_SHIFT); break;
					default: System.err.println("error character: " + c); return;
				}
			}
		}
		keys.add(key);
