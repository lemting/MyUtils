...
public class ErcodeUtil {
	...
	public static void create(String path, String data, String format, int version, int level, int width, int height, int magin) {
		...
		switch (level) {
			...
			case 2:
			default:
				errorCorrectionLevel = ErrorCorrectionLevel.M; // 默认M级
		}
		...
		try {
			// 获取BitMatrix对象
			BitMatrix bitMatrix = new MultiFormatWriter().encode(data, BarcodeFormat.QR_CODE, width <= 0 ? 300 : width,
					height <= 0 ? 300 : height, hints);
