 ...
 * 
 * 		Point subPosition(Point start,Point end): 获取实际坐标的相对坐标
 * 		Point addPosition(Point start,Point deviation): 获取相对坐标的实际坐标
 * 		Point addPosition(Point start,int x,int y)
 * 		
 * 		boolean isMouseMove(Point old,int m,int n): 判断鼠标是否移动(old: 一开始的位置; m,n: x,y坐标允许的偏移)
 * 		boolean equalsColor(Color color,int n): 鼠标位置是否为指定颜色(color: 指定要比较的颜色 ; n: r,g,b允许的偏移)
 * 		boolean equalsColor(Color color,int r,int g,int b)
 * 		boolean equalsColor(Point now,Color color,int r,int g,int b): 指定位置是否为指定颜色
 * 	  ----------------------------------------------------------------------
 */
...
public class RobotUp
{
	...
	protected static Point nowPoint;
	//初始化 robot
	...
	/**
	 * 获取相对坐标的实际坐标
	 * @param start	相对坐标的坐标原点
	 * @param deviation 相对坐标
	 * @return 实际坐标
	 */
